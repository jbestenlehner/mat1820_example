...
y, delta_y  = 12, 0.4
z = x + y
delta_z = (delta_x**2 + delta_y**2)**0.5
...
# Plot for comparison
plt.plot(temp_rec_K, lnD_SI, 'ro')
plt.plot(temp_rec_K, np.polyval(p,temp_rec_K), 'k-')
plt.plot(temp_rec_K, np.polyval(p1,temp_rec_K), 'b--')
plt.show()
plt.close()
```
...
```python
p1, p1cov = np.polyfit(temp_rec_K, lnD_SI, 1, w = 1/lnD_SI_err, cov='unscaled')
```
...
print('lnD_0 = {:.3f} +/- {:.3f}'.format(p1[1], p1err[1]))
